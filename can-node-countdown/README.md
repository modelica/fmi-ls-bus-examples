# CAN Node FMU with Countdown clocks

This directory contains a demo FMU representing a CAN node which uses a countdown clock.

The CAN node periodically sends a CAN frame with ID `0x1` and 4 bytes of payload every 300ms.

The FMU requires a simulator with support for countdown clocks, event mode and variable step size.
